...
func (r *etcd) register(addr string, ttl int) error {
	...
	_, err = r.cli.Put(context.Background(), "/"+discovery.Prefix+"/"+r.serviceName+"/"+addr, addr, clientv3.WithLease(leaseResp.ID))
	if err != nil {
		return err
	}
...
func (r *etcd) UnRegister(addr string) error {
	_, _ = r.cli.Delete(context.Background(), "/"+discovery.Prefix+"/"+r.serviceName+"/"+addr)
	r.shutdown <- "stop"
	return nil
}
